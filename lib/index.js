...
class PhpUnitStyleReporter {
    ...
    beforeItem(err, args) {
        this.currItem = {name: args.item.name, passed: true, failedAssertion: null};
    }
    ...
    assertion(err, args) {
        if (err) {
            ...
            assertion.message = args.assertion;
            assertion.error = args.error;
            this.currItem.failedAssertion = assertion;
        }
    }
    ...
    item(err, args) {
        ...
        if(this.failures.length > 0) {
            ...
            for(var i = 0; i < this.failures.length; i++) {
                ...
                this.writeRed('Failed assertion: ');
                console.log(this.failures[i].item.failedAssertions[0].message);
                this.writeRed('Reason: ');
                console.log(this.failures[i].item.failedAssertions[0].error.message);
